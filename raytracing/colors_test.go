...
func TestColors(t *testing.T) {
	...
	t.Run("Adding colors", func(t *testing.T) {
		...
		AssertColorsEqual(t, Add(c1, c2), RGB{1.6, 0.7, 1.0})
	})
	...
	t.Run("Subtracting colors", func(t *testing.T) {
		...
		AssertColorsEqual(t, Subtract(c1, c2), RGB{0.2, 0.5, 0.5})
	})

	t.Run("Multiply a color by a scalar", func(t *testing.T) {
		c := RGB{0.2, 0.3, 0.4}

		AssertColorsEqual(t, c.Multiply(2.), RGB{0.4, 0.6, 0.8})
	})
	...
	t.Run("Multiply 2 colors together", func(t *testing.T) {
		...
		AssertColorsEqual(t, Hadamard(c1, c2), RGB{0.9, 0.2, 0.04})
	})
	...
	t.Run("Color add is variadic", func(t *testing.T) {
		...
		AssertColorsEqual(t, Add(c1, c2, c3), RGB{5, 6, 5})
	})
